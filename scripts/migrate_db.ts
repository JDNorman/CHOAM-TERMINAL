...
const main = async () => {
  ...
  console.log(`Found ${persMsg} persistent messages to migrate`);
  ...
  for (const msg of persMsg) {
    count++;
    console.log(`Migrating persistent messages ${count}/${persMsg.length}`);
  ...
  for (const prev of prevData) {
    count++;
    console.log(`Migrating persistent messages ${count}/${prevData.length}`);
    const data = db.query.persistentMessages.findFirst({
